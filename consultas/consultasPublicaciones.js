...
//Mostrar publicaciones como galería
const filtrarGaleria = async ({ limit = 8, page = 1, categoria_id }) => {
  //Consulta para contar el número total de filas en la tabla 'publicaciones'
  const countQuery = "SELECT * FROM publicaciones";
  const { rows: countResult } = await pool.query(countQuery);
  const total_rows = parseInt(countResult[0].count, 10);
  ...
  const query = `
    ...
    ON 
        p.id = i.publicacion_id
    WHERE p.categoria_id = %s LIMIT %s OFFSET %s;
`;
  const offset = (page - 1) * limit;
  const formattedQuery = format(query, categoriaId, limit, offset);
  const { rows } = await pool.query(formattedQuery);
...
export const consultasPublicaciones = { obtenerGaleria, agregarPublicacionDB, detallePublicacionDB, publicacionesUsuarioDB, eliminarPublicacionDB, filtrarGaleria }